...
router.post('/children/link', authenticate, requireRole(ROLES.PARENT), asyncHandler(async (req, res) => {
  ...
  const result = await transaction(async (conn) => {
    const [codeRows] = await conn.execute(
      `SELECT ac.id, ac.used_by FROM access_code ac WHERE ac.code = ?`,
    ...
    const [childRows] = await conn.execute(
      `SELECT u.id, u.full_name, u.status FROM app_user u WHERE u.id = ? AND u.role = ?`,
    ...
    const [mine] = await conn.execute(
      'SELECT id FROM parent_link WHERE parent_user_id = ? FOR UPDATE',
      [req.user.id],
    );
    ...
    const [theirs] = await conn.execute(
      'SELECT id FROM parent_link WHERE student_user_id = ? FOR UPDATE',
      [child.id],
    );
    ...
    try {
      await conn.execute(
        `INSERT INTO parent_link (parent_user_id, student_user_id, relation) VALUES (?, ?, ?)`,
...
router.get('/children', authenticate, requireRole(ROLES.PARENT), asyncHandler(async (req, res) => {
  const rows = await query(
    `SELECT u.id, u.full_name, u.status, u.last_login_at, u.created_at,
            ...
            (SELECT COUNT(*) FROM content_progress p WHERE p.user_id = u.id) AS started,
            (SELECT COUNT(*) FROM content_progress p WHERE p.user_id = u.id AND p.status = 'completed') AS completed
       FROM parent_link pl
       JOIN app_user u              ON u.id = pl.student_user_id
  ...
  res.json({
    children: rows.map((r) => ({
      ...
      accessCode: r.access_code, registeredAt: r.created_at, lastLoginAt: r.last_login_at,
...
router.get('/children/:id/activity', authenticate, requireRole(ROLES.PARENT), asyncHandler(async (req, res) => {
  ...
  const link = await one(
    'SELECT id FROM parent_link WHERE parent_user_id = ? AND student_user_id = ?',
...
router.delete('/children/:id', authenticate, requireRole(ROLES.PARENT), asyncHandler(async (req, res) => {
  const result = await execute(
    'DELETE FROM parent_link WHERE parent_user_id = ? AND student_user_id = ?',
    [req.user.id, req.params.id],
  );
